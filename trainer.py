...
from Speecher.losses import calculate_loss, calculate_d_loss
        ...
        for train_steps_per_epoch, batch in enumerate(tqdm(self.train_dataloader, desc="[train]"), 1):
            ...
            if scaler is not None:
                with torch.cuda.amp.autocast():
                    d_loss, d_loss_latent = calculate_d_loss(self.model, self.discriminator, self.args, x_real, spk_emb)
            ...
            if scaler is not None:
                with torch.autograd.set_detect_anomaly(True):
                    with torch.cuda.amp.autocast():
                        loss, loss_latent = calculate_loss(self.model, self.discriminator, self.args, x_real, spk_emb)
                    with torch.autograd.set_detect_anomaly(True):
    ...
    def _sample_write_epoch(self, sample_write_params, vocoder, device='cpu',):
        if self.save_samples and self.gen_dataloader is not None and self.epochs % 2 == 0:
            ...
            for gen_steps_per_epoch, batch in enumerate(tqdm(self.gen_dataloader, desc="[generate sample]"), 1):
                ...
                x_fake = self.model(real, spk_emb)
